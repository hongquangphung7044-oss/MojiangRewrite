                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(12.dp), modifier = Modifier.fillMaxWidth()) {
                        Button(
                            onClick = {
                                ...
                                LocalNovelStore.applyFakeRewrite(chapters.map { it.id }, effectivePrompt, (intensity * 100).toInt())
                            },
                            enabled = novel != null,
                            modifier = Modifier.weight(1f)
                        ) { Text("开始处理范围") }
                        OutlinedButton(
                            onClick = {
                                ...
                                LocalNovelStore.applyFakeRewrite(listOfNotNull(chapter?.id), effectivePrompt, (intensity * 100).toInt())
                            },
                            enabled = novel != null,
                            modifier = Modifier.weight(1f)
                        ) { Text("预览首章效果") }
